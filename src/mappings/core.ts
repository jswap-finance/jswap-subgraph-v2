...
import { updatePairDayData, updateTokenDayData, updateJswapDayData, updatePairHourData, updateTokenHourData } from './dayUpdates'
...
export function handleTransfer(event: Transfer): void {
  ...
  let factory = JswapFactory.load(FACTORY_ADDRESS) as JswapFactory
...
export function handleSync(event: Sync): void {
  ...
  let jswap = JswapFactory.load(FACTORY_ADDRESS) as JswapFactory

  // reset factory liquidity by subtracting onluy tarcked liquidity
  jswap.totalLiquidityETH = jswap.totalLiquidityETH.minus(pair.trackedReserveETH as BigDecimal)
  ...
  jswap.totalLiquidityETH = jswap.totalLiquidityETH.plus(trackedLiquidityETH)
  jswap.totalLiquidityUSD = jswap.totalLiquidityETH.times(bundle.ethPrice)
  ...
  pair.syncBlockNumber = event.block.number
  jswap.syncBlockNumber = event.block.number
...
export function handleMint(event: Mint): void {
  ...
  jswap.txCount = jswap.txCount.plus(ONE_BI)
...
export function handleSwap(event: Swap): void {
  ...
  let jswap = JswapFactory.load(FACTORY_ADDRESS) as JswapFactory
  jswap.totalVolumeUSD = jswap.totalVolumeUSD.plus(trackedAmountUSD)
  jswap.totalVolumeETH = jswap.totalVolumeETH.plus(trackedAmountETH)
  jswap.untrackedVolumeUSD = jswap.untrackedVolumeUSD.plus(derivedAmountUSD)
  jswap.txCount = jswap.txCount.plus(ONE_BI)
  ...
  let jswapDayData = updateJswapDayData(event)
  ...
  jswapDayData.totalVolumeETH = jswap.totalVolumeETH
  jswapDayData.totalVolumeUSD = jswap.totalVolumeUSD
  jswapDayData.dailyVolumeUSD = jswapDayData.dailyVolumeUSD.plus(trackedAmountUSD)
  jswapDayData.dailyVolumeETH = jswapDayData.dailyVolumeETH.plus(trackedAmountETH)
  jswapDayData.dailyVolumeUntracked = jswapDayData.dailyVolumeUntracked.plus(derivedAmountUSD)
  jswapDayData.save()
